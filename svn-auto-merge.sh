...
VER=$(svn log --stop-on-copy ${BRANCH} | awk '/^r/ {print $1}' | tail -1)
echo -e "Merge branch ${GREEN}${BRANCH}${CCLR} from ${GREEN}${VER}${CCLR} to ${GREEN}HEAD${CCLR}"
svn merge -${VER}:HEAD ${BRANCH} ${WORKDIR} >/dev/null
...
if [ "${REPLY}" == 'y' -o "${REPLY}" == 'yes' ]; then
    ...
    echo "You can try ${GREEN}svn-override-merge.sh${CCLR}"
fi
